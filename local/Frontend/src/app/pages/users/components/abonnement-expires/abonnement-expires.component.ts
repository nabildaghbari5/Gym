...
@Component({
  selector: 'app-abonnement-expires',
  templateUrl: './abonnement-expires.component.html',
  styleUrl: './abonnement-expires.component.scss'
})
export class AbonnementExpiresComponent  {
 ...
 findAllAdherent(){
  this.adherentService.findUsersByStatus(this.pageNumber , this.pageSize ,"Expiré" ).subscribe({ 
    next:(adherents) => {
 ...
 OnSave(){
    ...
    this.adherentService.updateAbonnement(this.idAdherent,formData).subscribe({
        next:(data)=> { 
          this.findAllAdherent();
